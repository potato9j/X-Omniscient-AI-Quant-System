...
다른 PC나 제3자 그리고 Codex/Claude 작업공간에서 이어받을 때는 먼저 [`PROJECT_HANDOFF.md`](PROJECT_HANDOFF.md)를 확인하십시오. 전체 목표, 진행 타임라인, 로컬 DB 주의사항, 다음 작업 순서가 정리되어 있습니다.
...
- 외부 증권 API는 시세/지수/분봉 조회 전용으로만 사용합니다.
- 매수, 매도, 정정, 취소, 자동매매 기능은 제품 범위에서 제외합니다.
...
이 시스템은 투자 판단을 보조하는 데이터/AI 대시보드입니다. 실거래 주문 기능은 현재뿐 아니라 향후 출시 범위에서도 제외합니다.
...
외부 API를 붙일 때도 주문 관련 endpoint, 계좌 잔고/주문 가능 금액 조회, 실거래 권한은 사용하지 않습니다.
...
향후 한국투자증권 Open API를 붙일 경우 다음 값만 시세 조회 전용으로 사용합니다.

- `KIS_APP_KEY`
- `KIS_APP_SECRET`
- `KIS_BASE_URL`
- 접근 토큰 캐시 경로 또는 로컬 설정

계좌번호, 주문 비밀번호, 주문 endpoint 설정은 사용하지 않습니다. API 키는 `.env`, 로컬 설정 파일, 또는 OS 보안 저장소에만 보관하고 Git에 커밋하지 않습니다.
...
5. 장중/장마감 시세를 읽기 전용 API로 자동 동기화합니다.
6. 대시보드는 가격, 뉴스, 감성, 방향 확률, 예상 수익률, 목표가를 하나의 화면에 표시합니다.
